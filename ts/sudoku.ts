...
class CampoMinado {
	...
	public geraCampo(nMinas:number, ci:number, cj:number) {		
		for (let n = 0; n < nMinas; n++) { // colocando as minas
			while (true) {
				let i = global.randInt(0, this.nLines-1);
				let j = global.randInt(0, this.nColumns-1);

				// se [i,j] está envolta do click inicial, procura outro ponto
				if (global.inRange(i, ci-1, ci+1) && global.inRange(j, cj-1, cj+1)) { 
					continue;
				}

				// verifica se a celula já nn é mina, se nn for coloca a prox.
...
function main() {
	...
	NMINAS = Number(params.get('minas'));
	let cols:number = Number(params.get('cols'));
	let rows:number = Number(params.get('rows'));

	gameover = GameOver.no;	
	nbands = NMINAS;

	cm = new CampoMinado(rows, cols);
	firstClick = true;
	// cm.geraCampo(NMINAS);
